...
from Savage_Char_Datenbasis_0_6 import *
...
class CharakterView:
   ...
   # Funktion zum Berechnen der Traglast basierend auf der Stärke
    ...
    def eingabe_nahkampf_waffe(self):
        ...
        for i, waffe in enumerate(self.model.nahkampf_waffe):
            selected = tk.BooleanVar(value=False)
            self.charakter_daten_selected_fertigkeiten[waffe] = selected
            checkbox = tk.Checkbutton(frame, text=waffe, variable=selected)
            checkbox.grid(row=i, column=0, sticky='w')

        tk.Button(frame, text="Speichern", command=self.speichern_nahkampf_waffe).grid(row=len(self.model.nahkampf_waffe), column=0)

    @Fehlerbehandlung.handle_errors
    def speichern_nahkampf_waffe(self):
        self.model.charakter_daten['Nahkampfwaffen'] = {waffe: True for waffe in self.model.nahkampf_waffe if self.charakter_daten_selected_fertigkeiten[waffe].get()}
        self.model.speichern()
        self.ausgabe_fenster(self.model.charakter_daten)
        messagebox.showinfo("Erfolg", "Nahkampfwaffen gespeichert!")
    ...
    def ausgabe_fenster(self, charakter_daten):
        ...
        for waffe, details in charakter_daten['Nahkampfwaffen'].items():
            tk.Label(waffe_frame, text=f"{waffe} (Schaden: {details['Schaden']}, Mindeststärke: {details['Mindeststärke']}, Gewicht: {details['Gewicht']}, Kosten: {details['Kosten']})", bg='#F0F0F0').pack(anchor='w')
