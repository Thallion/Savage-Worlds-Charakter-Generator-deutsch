...
from Savage_Char_Datenbasis_0_9 import *
...
class CharakterView:
   ...
   # Funktion zum Berechnen der Traglast basierend auf der Stärke
    ...
    @Fehlerbehandlung.handle_errors
    def eingabe_fernkampf_waffe(self):
        frame = self.add_scrollbar_to_tab('Fernkampfwaffen')
    ...
    def ausgabe_fenster(self, charakter_daten):
        ...
        for waffe, details in charakter_daten['Nahkampfwaffen'].items():
            tk.Label(waffe_frame, text=f"{waffe} (Schaden: {details['Schaden']}, Reichweite: {details['Reichweite']}, PB: {details['PB']}, FR: {details['FR']}, Schuss: {details['Schuss']}, Mindeststärke: {details['Mindeststärke']})", bg='#F0F0F0').pack(anchor='w')
        for waffe, details in charakter_daten['Fernkampfwaffen'].items():
            tk.Label(waffe_frame, text=f"{waffe} (Schaden: {details['Schaden']}, Reichweite: {details['Reichweite']}, PB: {details['PB']}, FR: {details['FR']}, Schuss: {details['Schuss']}, Mindeststärke: {details['Mindeststärke']})", bg='#F0F0F0').pack(anchor='w')
