...
def _find_build_roots():
    """Findet alle Build-Verzeichnisse dynamisch (Single- oder Multi-Arch)"""
...
def fix_kivy_python3_compatibility():
    ...
    # Find kivy build directories in all build roots
    build_roots = _find_build_roots()
    if not build_roots:
    ...
    for build_root in build_roots:
        kivy_dirs.extend(list(build_root.glob("kivy*/*/kivy")))
...
def fix_pyjnius_python3_compatibility():
    ...
    pyjnius_dirs = []
    for build_root in build_roots:
        pyjnius_dirs.extend(list(build_root.glob("pyjnius*/*/pyjnius")))
